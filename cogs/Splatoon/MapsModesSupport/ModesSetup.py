...
class MapsModesSetup:
    ...
    async def big_run(self) -> BigRun | None:
        ...
        big_run_info = BigRun(time = TimeSlots(start = self.generate_timestamp(big_run['startTime']), end = self.generate_timestamp(big_run(big_run['endTime']))),
    ...
    async def eggstra_work(self) -> EggstraWork | None:
        ...
        eggstra_work_info = EggstraWork(time = TimeSlots(start = await self.generate_timestamp(eggstra_work['startTime']), end = await self.generate_timestamp(eggstra_work['endTime'])),
